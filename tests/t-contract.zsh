#!/usr/bin/env zunit
#{{{                    MARK:Header
##### Purpose: zsh-sudo — plugin-contract pins.
...
@test 'entrypoint stem matches plugin directory basename' {
    ...
    # stem of `sudo.plugin.zsh` must equal the parent directory's
    # basename so generated source lines stay copy-pasteable.
    local entry='sudo.plugin.zsh'
...
@test 'entrypoint parses cleanly under zsh -n' {
    run zsh -n "$pluginDir/sudo.plugin.zsh"
    assert $state equals 0
}
